...
const translations = {
    ...
    nav: {
        id: {
            ...
            oee_education: 'OEE Education (Pusat Edukasi)',
        ...
        ja: {
            ...
            reports: '帳票・実績日報出力',
            master_data: 'マスタ管理',
            oee_education: 'OEE 基礎教育・学習ガイド',
            section_system: 'システム管理・権限',
            users: 'ユーザー権限管理',
            audit_trail: '監査ログ (Audit Trail)',
            settings: 'システム環境設定',
            database: 'データベース管理',
...
const MONOZUKURI_MASTER_DICTIONARY = [
    // --- DATABASE MANAGEMENT & SELECTIVE GO-LIVE CLEANUP MODULE ---
    ["Manajemen Database & Pembersihan Go-Live", "データベース管理及び本番稼働前データ初期化"],
    ["Diagnostik spesifikasi teknis database, informasi server runtime, dan pembersihan massal data uji coba untuk persiapan Live.", "データベース技術仕様・サーバー稼働環境診断、及び本番稼働に向けたテストデータ選択初期化。"],
    ["Export Backup (JSON)", "JSONバックアップ出力"],
    ["Optimasi & Defrag DB", "DB最適化・デフラグ"],
    ["Pembersihan Data Massal Persiapan Go-Live (Selective Go-Live Data Cleanup)", "本番稼働に向けたトランザクション選択初期化 (Go-Live Data Cleanup)"],
    ["Pembersihan Data Massal untuk Persiapan Live (Go-Live Preparation)", "本番稼働に向けたトランザクション初期化 (Go-Live Data Cleanup)"],
    ["Pilih secara spesifik tabel data transaksi uji coba yang ingin dibersihkan dengan checklist di bawah sebelum sistem dijalankan secara resmi.", "システム正式稼働前に、以下のチェックリストから初期化対象のテスト用トランザクションテーブルを選択してください。"],
    ["Bersihkan data uji coba / simulasi agar aplikasi bersih dan siap digunakan secara resmi oleh operator & supervisor lini.", "テスト用データを初期化し、正式運用に向けてデータベースをクリーンな状態にします。"],
    ["Master Data Shield: AKTIF & TERPROTEKSI", "マスタデータ保護シールド: 有効・完全保護"],
    ["Master Data Shield: Aktif & Terproteksi", "マスタデータ保護シールド: 有効・完全保護"],
    ["Jaminan Keamanan Sistem & Proteksi Master Data", "システム安全性保証及びマスタデータ完全保護"],
    ["Jaminan Keamanan Master Data", "マスタデータ安全性保証"],
    ["Pilihan Cepat:", "クイック選択:"],
    ["Pilih Semua Transaksi", "全トランザクション選択"],
    ["Kosongkan Pilihan", "選択を全解除"],
    ["Data Produksi & OEE", "生産実績及びOEE"],
    ["Log Downtime & Trouble", "設備停止及びトラブルログ"],
    ["Defect & Mutu NG", "不良明細及び品質検査"],
    ["Sensor & Event IoT", "IoTセンサー及び稼働イベント"],
    ["Bersihkan Tabel Terpilih Sesuai Checklist", "選択したテーブルを一括初期化"],
    ["Checklist Tabel Data Transaksional", "トランザクションデータテーブル一覧 (選択初期化)"],
    ["Tabel Data Transaksional", "トランザクションデータテーブル"],
    ["Centang tabel yang ingin dibersihkan datanya untuk persiapan Live.", "初期化したいトランザクションテーブルにチェックを入れてください。"],
    ["PILIH / CHECKLIST", "選択チェックリスト"],
    ["Deskripsi Fungsi Transaksi", "トランザクション機能概要"],
    ["Status Aksi", "実行ステータス"],
    ["DIBERSIHKAN", "初期化対象"],
    ["DILEWATI (AMAN)", "除外 (保護)"],
    ["Tabel Master Data & Profil Terproteksi", "保護対象マスタデータ及び会社設定テーブル"],
    ["Tabel Master & Konfigurasi", "マスタデータ及び環境設定テーブル"],
    ["Master data, target OEE, dan profil perusahaan dijamin kebal dan tidak dapat dihapus.", "マスタデータ、目標設定、および会社情報は完全に保護され削除できません。"],
    ["100% TERPROTEKSI", "100% 完全保護"],
    ["TERPROTEKSI", "完全保護"],
    ["Nama Tabel Master", "マスタテーブル名"],
    ["Kategori & Deskripsi Master Data", "マスタ分類及び機能"],
    ["Kategori Master", "マスタ分類"],
    ["Status Perlindungan", "保護状態"],
    ["AMAN / PERMANEN", "安全 / 恒久保持"],
    ["Konfirmasi Pembersihan Selektif Data Go-Live", "本番稼働前 選択データ初期化の確認"],
    ["Konfirmasi Pembersihan Data Go-Live", "本番稼働前 データ初期化の確認"],
    ["Tindakan penghapusan data uji coba memerlukan verifikasi frasa keamanan.", "テストデータの初期化を実行するには、安全確認フレーズの入力が必要です。"],
    ["Daftar Tabel yang Akan Dikosongkan:", "初期化されるテーブル一覧:"],
    ["Tabel Terpilih", "テーブル選択"],
    ["Total Data Terhapus:", "初期化される総データ件数:"],
    ["Master Data & Pengaturan Terproteksi 100%", "マスタデータ及びシステム設定は100%保護されます"],
    ["Ketik BERSIHKAN di sini...", "ここに「BERSIHKAN」または「CONFIRM」と入力..."],
    ["Konfirmasi & Bersihkan Data Terpilih", "選択したデータを完全に初期化する"],
    ["Konfirmasi & Bersihkan Data", "データを初期化する"],
    ["Pembersihan Total Transaksi", "全トランザクション完全初期化"],
    ["Bersihkan Semua Transaksi", "全トランザクションを初期化"],
    ["REKOMENDASI GO-LIVE", "本番稼働推奨"],
    ["Data Laporan Produksi", "生産日報・実績データ"],
    ["Bersihkan Data Produksi", "生産実績データを初期化"],
    ["Log Trouble & Downtime", "設備停止トラブルログ"],
    ["Bersihkan Log Trouble", "トラブルログを初期化"],
    ["Rekap Defect & NG Mutu", "不良集計及びQC検査データ"],
    ["Bersihkan Data Defect NG", "不良データを初期化"],

    // --- AUDIT TRAIL & SYSTEM INTEGRITY LOGGING MODULE ---
    ["Standar Audit TI Nasional • ISO 27001:2022", "国家IT監査基準 • ISO/IEC 27001:2022"],
    ["Standar Audit TI Nasional", "国家IT監査基準"],
    ["Audit Trail & Log Integritas Sistem", "監査証跡・システム完全性ログ (Audit Trail)"],
    ["Audit Trail & Log Integritas", "監査証跡・完全性ログ"],
    ["Audit Trail", "監査ログ (Audit Trail)"],
    ["Rekaman jejak aktivitas operasional, perubahan nilai data", "PT Yasunaga Indonesiaのオンプレミス社内サーバー運用監視、データ変更履歴"],
    ["Before & After Diff", "変更前後の差分 (Before & After Diff)"],
    ["serta diagnosa hosting server internal untuk pemantauan kendala server on-premise di PT Yasunaga Indonesia.", "およびシステム操作ログを記録。"],
    ["Simulasi Log Audit", "監査ログ模擬生成 (Simulation)"],
    ["Ekspor Excel", "Excelエクスポート"],
    ["Cetak Dokumen", "監査帳票印刷 (PDF)"],
    ["Muat Ulang Log Audit", "監査ログ再読込"],
    ["Total Log Audit", "監査ログ総件数"],
    ["Akun Terdaftar Beraktivitas", "名のアクティブ登録アカウント"],
    ["Aktivitas Hari Ini", "本日のシステム操作"],
    ["Realtime Session Transaksi", "リアルタイムトランザクション"],
    ["Peringatan / Kritis", "警告・重要イベント"],
    ["Hapus Data & Perubahan Role", "データ削除及び権限変更"],
    ["Server Internal LAN", "社内LANサーバー"],
    ["Log Transaksi & Inspeksi Diff (Before & After)", "操作ログ及び変更差分検証 (Before & After)"],
    ["Diagnosa Hosting Server Internal (On-Premise)", "社内オンプレミスサーバー診断 (On-Premise)"],
    ["Diagnosa Hosting Server Internal", "社内オンプレミスサーバー診断"],
    ["Cari pengguna, deskripsi, IP, entri ID...", "ユーザー名、操作内容、IP、ID等で検索..."],
    ["Semua Modul", "全モジュール"],
    ["Produksi & OEE", "生産実績・OEE"],
    ["Detail Reject NG", "不良明細 (NG Detail)"],
    ["Downtime Mesin", "設備停止 (Downtime)"],
    ["Keamanan & Autentikasi", "セキュリティ・認証"],
    ["Semua Tipe Aksi", "全操作種別"],
    ["CREATE (Tambah Data)", "CREATE (新規登録)"],
    ["UPDATE (Ubah Data)", "UPDATE (データ更新)"],
    ["DELETE (Hapus Data)", "DELETE (データ削除)"],
    ["LOGIN / AUTH", "LOGIN / ログイン認証"],
    ["CONFIG (Pengaturan)", "CONFIG (設定変更)"],
    ["BACKUP / RESTORE", "BACKUP / バックアップ"],
    ["Semua Severity", "全重要度 (Severity)"],
    ["Danger / Kritis", "重要・削除 (Danger)"],
    ["Reset Filter", "条件リセット"],
    ["Waktu (WIB)", "記録日時 (WIB)"],
    ["Pengguna & Peran", "操作ユーザー・権限"],
    ["Modul & Target ID", "対象モジュール・ID"],
    ["Deskripsi & Rincian Perubahan", "操作概要・変更詳細"],
    ["IP & Jaringan", "IPアドレス・端末"],
    ["Status Integritas", "暗号検証状態"],
    ["Inspeksi Diff", "差分検証"],
    ["Belum Ada Rekaman Log Audit", "監査ログデータがありません"],
    ["Klik tombol \"Simulasi Log Audit\" di kanan atas untuk membuat contoh log real-world standar audit nasional.", "右上の「監査ログ模擬生成」ボタンをクリックして、監査標準サンプルのログを作成してください。"],
    ["field nilai berubah (Before & After tercatat)", "項目の値が変更されました (Before/After記録済)"],
    ["Menampilkan", "表示中:"],
    ["dari", "/ 全"],
    ["rekaman log", "件"],
    ["Hal", "ページ"],
    ["Host Server & Runtime", "サーバーホスト及び稼働環境"],
    ["Spesifikasi hosting server internal pabrik", "工場内オンプレミスサーバー仕様"],
    ["Host Address:", "ホストアドレス:"],
    ["Host Address", "ホストアドレス"],
    ["Sistem Operasi:", "OS (基本ソフト):"],
    ["Sistem Operasi", "OS (基本ソフト)"],
    ["PHP Version:", "PHPバージョン:"],
    ["Framework MES:", "MESフレームワーク:"],
    ["Framework MES", "MESフレームワーク"],
    ["Zona Waktu Server:", "サーバータイムゾーン:"],
    ["Zona Waktu Server", "サーバータイムゾーン"],
    ["Kapasitas Storage & Memori", "ストレージ容量及びメモリ使用状況"],
    ["Alokasi memori runtime & ruang disk", "実行メモリ割当及びディスク空き容量"],
    ["Penggunaan Disk Penyimpanan:", "ディスク使用率:"],
    ["Penggunaan Disk Penyimpanan", "ディスク使用率"],
    ["Memory Usage (PHP):", "PHPメモリ使用量:"],
    ["Memory Peak:", "最大ピークメモリ:"],
    ["Memory Limit:", "メモリ上限設定:"],
    ["Sisa", "空き"],
    ["Integritas Audit & Database", "監査証跡完全性及びDB状態"],
    ["Standar keamanan BSSN & ISO 27001", "ISO 27001及びセキュリティ基準準拠"],
    ["Database Engine:", "DBエンジン:"],
    ["Ukuran Database:", "データベース容量:"],
    ["Ukuran Database", "データベース容量"],
    ["Hash Chaining:", "暗号ハッシュ連鎖:"],
    ["Status Sertifikasi:", "認証ステータス:"],
    ["Inspeksi Perubahan Data (Before & After)", "データ変更差分検証 (Before & After)"],
    ["Inspeksi Perubahan Data", "データ変更差分検証"],
    ["Aktivitas yang Tercatat:", "記録された操作内容:"],
    ["Nilai Sebelum (Before / Old)", "変更前の値 (Before / Old)"],
    ["Nilai Sesudah (After / New)", "変更後の値 (After / New)"],
    ["DATA AWAL", "初期データ"],
    ["NULL / DATA BARU", "新規登録データ"],
    ["DATA TERBARU", "更新後データ"],
    ["NULL / DIHAPUS", "データ削除"],
    ["(Tidak ada data sebelum / Entri baru)", "(変更前データなし / 新規登録)"],
    ["(Data dihapus permanen)", "(データ完全削除)"],
    ["Matriks Perbandingan Properti / Field", "項目別変更比較マトリクス"],
    ["Kolom Berubah", "項目変更"],
    ["Nama Field", "項目名 (Field)"],
    ["Nilai Lama (Before)", "変更前の値 (Before)"],
    ["Nilai Baru (After)", "変更後の値 (After)"],
    ["Sertifikat Digital Integritas Log (SHA-256 Checksum)", "電子署名・ログ完全性証明 (SHA-256 Checksum)"],
    ["Waktu Eksekusi / Latency:", "処理所要時間 / 応答性:"],
    ["Tutup Inspeksi", "閉じる"],
    ["Simulasi Log Audit Sistem", "監査ログ模擬生成の確認"],
    ["Apakah Anda ingin menambahkan contoh rekaman log audit real-world (Produksi OEE, Reject NG, Downtime, User Role, & Setting) untuk keperluan simulasi audit?", "監査機能テスト用として、生産実績・不良明細・停止履歴・ユーザー権限・システム設定の実践的な模擬ログを生成しますか？"],
    ["Ya, Buat Simulasi Log", "はい、模擬ログを生成する"],
    ["Log audit simulasi standar nasional berhasil ditambahkan!", "国家監査基準に準拠した模擬ログが正常に生成されました！"],
    ["Simulasi Berhasil", "模擬ログ生成完了"],
    ["Gagal memuat log audit sistem", "監査ログの読み込みに失敗しました"],
    ["Memuat status database & environment...", "データベース及びサーバー環境ステータスを読み込み中..."],
    ["Memuat rekaman log audit sistem & kriptografi hash...", "システム監査ログ及び暗号ハッシュを読み込み中..."],
    ["DOKUMEN RESMI AUDIT TRAIL & INTEGRITAS SISTEM (ISO 27001 / BSSN)", "公式システム監査証跡及び完全性報告書 (ISO 27001/国家基準)"],
    ...
    ["Form: A4-Portrait Trouble & Downtime Report", "様式: A4縦 設備停止・トラブル実績報告書"],
    ["Form: A4-Portrait Shift Handover", "様式: A4縦 直間引継ぎ報告書"],
    ["Form: A4-Landscape QC Defect Inspection Report", "様式: A4横 品質検査・不良分析報告書"],
    ["Form: A4-Portrait QC Defect Inspection Report", "様式: A4縦 品質検査・不良分析報告書"],
